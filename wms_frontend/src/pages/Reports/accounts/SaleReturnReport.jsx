...
//        try {
//            const res = await reportApi.getSaleReturnReport(fromDate, toDate, selectedCustomer || null, selectedItem || null);
...
//    return (
//        <ReportTemplate title="SALE RETURN REPORT" subtitle="Summary & detail of sale returns" filters={filters} printedBy="admin"
...
//                <div key={r.returnTranNumb} style={{ marginBottom: 16 }}>
//                    <div style={{ display: 'flex', gap: 16, padding: '10px 14px', border: '1px solid #e5e7eb', borderRadius: 8, background: '#fafbfc', marginBottom: 4, cursor: 'pointer' }} onClick={() => toggleExpand(r.returnTranNumb)}>
//                        <div style={{ flex: 1 }}><strong>{r.billNumb}</strong></div>
//                        <div style={{ flex: 1 }}>{formatDt(r.tranDate)}</div>
//                        <div style={{ flex: 2 }}>{r.customerName}</div>
//                        <div style={{ flex: 1, textAlign: 'center' }}>{r.totalQty} qty</div>
//                        <div style={{ flex: 1, textAlign: 'right', fontWeight: 700 }}>{f(r.totalAmount)}</div>
//                        <div style={{ width: 24, textAlign: 'center', color: '#2563eb' }}>{expanded[r.returnTranNumb] ? <FaChevronDown /> : <FaChevronRight />}</div>
//                    </div>
//                    {expanded && expanded[r.returnTranNumb] && (
...
export default function SaleReturnReport() {
    ...
    const handleSearch = async () => {
        ...
        try {
            const res =
                await reportApi.getSaleReturnReport(
                    fromDate,
                    toDate,
    ...
    const exportPDF = () => {
        ...
        doc.text("SALE RETURN REPORT", pageWidth / 2, 12, { align: "center" });
        ...
        doc.save(`SaleReturnReport_${fromDate}_to_${toDate}.pdf`);
    };
    ...
    const exportExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Sale Return Report");
        const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
        const blob = new Blob([excelBuffer], { type: "application/octet-stream" });
        saveAs(blob, `SaleReturnReport_${fromDate}_to_${toDate}.xlsx`);
    };
    ...
    return (
        <ReportTemplate
            ...
            subtitle="Summary & detail of sale returns"
            filters={filters}

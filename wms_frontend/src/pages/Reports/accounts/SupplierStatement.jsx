...
export default function SupplierStatement() {
    ...
    const exportPDF = () => {
        ...
        doc.text("SUPPLIER STATEMENT REPORT", pageWidth / 2, 12, { align: "center" });

        doc.setFontSize(7);
        doc.setFont("helvetica", "normal");

        // Left side
        doc.text(`Branch: ${branchName}`, 8, 20);
        doc.text(`Supplier: ${supplierLabel}`, 8, 25);

        if (selectedSupplierData) {
            doc.text(`Opening: ${formatNumber(selectedSupplierData.openingBalance)} ${selectedSupplierData.openingBalanceType}`, 8, 30);
            doc.text(`Closing: ${formatNumber(selectedSupplierData.closingBalance)} ${selectedSupplierData.closingBalanceType}`, 8, 35);
        }

        // Right side
        doc.text(`Period: ${fromDate} to ${toDate}`, pageWidth - 8, 20, { align: "right" });
        doc.text(`Generated By: ${loggedInUser}`, pageWidth - 8, 25, { align: "right" });
        doc.text(`Generated On: ${generatedOn}`, pageWidth - 8, 30, { align: "right" });
        ...
        body.push([
            ...
            { content: selectedSupplierData ? `${formatNumber(selectedSupplierData.closingBalance)} ${selectedSupplierData.closingBalanceType}` : "", styles: { halign: "right", fontStyle: "bold", fontSize: 8 } }
        ]);
        ...
        autoTable(doc, {
            ...
            margin: { left: 6, right: 6, top: 5, bottom: 10 },
        ...
        doc.save(`SupplierStatement_${fromDate}_to_${toDate}.pdf`);
    };
